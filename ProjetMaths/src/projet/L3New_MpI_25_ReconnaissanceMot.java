...
public class L3New_MpI_25_ReconnaissanceMot {

	public static void reconnaissanceMot(L3New_MpI_25_Automate a) {
		...
		while (!entree.equals("fin")) {
			...
			if (index == -1) {
				...
				if (index < entree.toCharArray().length) {
					System.out.println(
							"Le premier caractere empechant le mot d'etre reconnu est :" + entree.toCharArray()[index]);
				} else {
	...
	public static int reconnaitre_mot(L3New_MpI_25_Automate a, String mot) {
	...
	public static int[] recursifTest(L3New_MpI_25_Automate a, char[] mot, String etat, int index) {
		index += 1;
		// si le mot est fini
		if (index == mot.length) {
